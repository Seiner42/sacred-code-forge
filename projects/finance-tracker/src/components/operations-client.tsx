...
const typeLabels = { expense: "Расход", income: "Пополнение", transfer: "Перевод" };
const months = [
...
function amountClass(kind: "expense" | "income" | "transfer") {
  if (kind === "expense") return "text-rose-300";
  if (kind === "income") return "text-emerald-300";
...
function formFromOperation(operation?: OperationItem, fallbackCategoryId?: string): FormState {
  if (!operation) return { operationDate: new Date().toISOString().slice(0, 10), merchant: "", amount: "", type: "expense", categoryId: fallbackCategoryId ?? "", description: "", includeInReports: true };
  return { operationDate: toComparableDate(operation.date), merchant: operation.merchant, amount: String(operation.amountValue), type: operation.type, categoryId: operation.categoryId ?? fallbackCategoryId ?? "", description: operation.description ?? "", includeInReports: operation.includeInReports };
}
...
export function OperationsClient({ transactions: initialTransactions, categories }: { transactions: OperationItem[]; categories: CategoryOption[] }) {
  ...
  const filteredTransactions = useMemo(() => transactions.filter((item) => {
    ...
    return true;
  }), [dateFrom, dateTo, hideTransfers, periodMode, selectedMonth, selectedYear, transactions, typeFilter]);

  function openCreate() { setEditingId(null); setForm(formFromOperation(undefined, categories[0]?.id)); setError(""); setIsModalOpen(true); }
  function openEdit(item: OperationItem) { setEditingId(item.id); setForm(formFromOperation(item, categories[0]?.id)); setError(""); setIsModalOpen(true); }
  function closeModal() { if (saving) return; setIsModalOpen(false); setEditingId(null); setForm(formFromOperation(undefined, categories[0]?.id)); setError(""); }

  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); setSaving(true); setError("");
    try {
      const response = await fetch(editingId ? `/api/transactions/${editingId}` : "/api/transactions", { method: editingId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ operationDate: form.operationDate, merchant: form.merchant, amount: Number(form.amount), type: form.type, categoryId: form.categoryId || null, description: form.description || null, includeInReports: form.includeInReports }) });
