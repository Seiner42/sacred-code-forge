...
export function OperationsClient({ transactions: initialTransactions, categories }: { transactions: OperationItem[]; categories: CategoryOption[] }) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      try {
        ...
        const result = await response.json() as { ok: boolean; suggestions?: MerchantSuggestion[] };
        if (!response.ok || !result.ok) {
          setMerchantSuggestions([]);
          setActiveSuggestionIndex(-1);
          return;
        }
        const suggestions = (result.suggestions ?? []).filter((item) => item.merchant !== query);
  ...
  function openCreate() { setEditingId(null); setForm(formFromOperation(undefined, categories[0]?.id)); setMerchantSuggestions([]); setActiveSuggestionIndex(-1); setIsMerchantFocused(false); setError(""); setIsModalOpen(true); }
  function openEdit(item: OperationItem) { setEditingId(item.id); setForm(formFromOperation(item, categories[0]?.id)); setMerchantSuggestions([]); setActiveSuggestionIndex(-1); setIsMerchantFocused(false); setError(""); setIsModalOpen(true); }
  function closeModal() { if (saving) return; setIsModalOpen(false); setEditingId(null); setForm(formFromOperation(undefined, categories[0]?.id)); setMerchantSuggestions([]); setActiveSuggestionIndex(-1); setIsMerchantFocused(false); setError(""); }
  ...
  function applyMerchantSuggestion(suggestion: MerchantSuggestion) {
    setForm((current) => ({
      ...current,
  ...
  function handleMerchantKeyDown(event: React.KeyboardEvent<HTMLInputElement>) {
    if (merchantSuggestions.length === 0) return;

    if (event.key === "ArrowDown") {
      event.preventDefault();
      setActiveSuggestionIndex((current) => (current + 1) % merchantSuggestions.length);
      return;
    }

    if (event.key === "ArrowUp") {
      event.preventDefault();
      setActiveSuggestionIndex((current) => (current <= 0 ? merchantSuggestions.length - 1 : current - 1));
      return;
    }

    if (event.key === "Enter" && activeSuggestionIndex >= 0) {
      event.preventDefault();
      applyMerchantSuggestion(merchantSuggestions[activeSuggestionIndex]!);
      return;
    }
