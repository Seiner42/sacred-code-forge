...
function buildForms(items: ReviewItem[], categories: CategoryOption[]) {
  return Object.fromEntries(items.map((item) => [item.id, { merchantNormalized: item.suggestedMerchantNormalized, categoryId: item.suggestedCategoryId ?? categories.find((category) => category.slug === "other")?.id ?? "", matchType: "contains" as const }]));
}
...
export function ReviewClient({ initialItems, categories }: { initialItems: ReviewItem[]; categories: CategoryOption[] }) {
  ...
  const [forms, setForms] = useState<Record<string, FormState>>(() => buildForms(initialItems, categories));
  ...
  async function handleRefreshReview() {
    ...
    setError("");
    try {
      const response = await fetch(`/api/review/refresh`, { method: "POST" });
      const result = (await response.json()) as { ok: boolean; items?: ReviewItem[]; error?: string };
      if (!response.ok || !result.ok || !result.items) {
        setError(result.error ?? "Не удалось обновить разбор");
        return;
      }
      setItems(result.items);
      setForms(buildForms(result.items, categories));
    } catch {
      setError("Сбой обновления разбора. Повтори попытку.");
    } finally {
