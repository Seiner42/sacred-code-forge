...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  db.prepare(`
    ...
    SET name = ?, period = ?, amount = ?, is_active = ?, charge_day = ?, charge_month = ?, updated_at = ?
    WHERE id = ?
  `).run(payload.name!.trim(), payload.period!, payload.amount!, payload.status === "active" ? 1 : 0, payload.chargeDay!, payload.period === "yearly" ? payload.chargeMonth! : null, new Date().toISOString(), id);
